...
var $author$project$RouteTest$testParse = F3(
	function (name, path, expectedRoute) {
		return A2(
			...
			function (_v0) {
				return A2(
					...
					A2(
						...
						$author$project$Route$parse,
						$elm$url$Url$fromString('http://example.com' + path)));
			});
	});
var $author$project$RouteTest$suite = A2(
	$elm_explorations$test$Test$describe,
	'Route',
	_List_fromArray(
		[
			A3(
			$author$project$RouteTest$testParse,
			'should parse Top',
			'/',
			$elm$core$Maybe$Just($author$project$Route$Top)),
			A3(
			$author$project$RouteTest$testParse,
			'should parse Top with queries',
			'/?dummy=value',
			$elm$core$Maybe$Just($author$project$Route$Top)),
			A3(
			$author$project$RouteTest$testParse,
			'should parse Top with hash',
			'/#dummy',
			$elm$core$Maybe$Just($author$project$Route$Top)),
			A3(
			$author$project$RouteTest$testParse,
			...
			'/foo',
			$elm$core$Maybe$Just(
				$author$project$Route$User('foo'))),
			A3(
			$author$project$RouteTest$testParse,
			'should parse Repo',
			'/foo/bar',
			$elm$core$Maybe$Just(
				A2($author$project$Route$Repo, 'foo', 'bar'))),
			A3($author$project$RouteTest$testParse, 'should parse invalid path', '/foo/bar/baz', $elm$core$Maybe$Nothing)
		]));
